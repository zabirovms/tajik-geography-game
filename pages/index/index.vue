  ...
  <view class="container">
    ...
    <view v-if="showGuide" class="guide-popup">
      <view class="guide-content">
        ...
        <text class="guide-description"
          >选择您感兴趣的游戏模式开始挑战，点击下方的分享按钮与朋友一起玩吧！</text
        >
    ...
    <view class="top-tips">
      <view class="tip-card" hover-class="tip-hover">
        ...
        <text class="tip-text"
          >欢迎来到地理知识挑战，选择您感兴趣的游戏模式开始挑战！</text
        >
      </view>
    </view>
    ...
    <view class="welcome-section">
      ...
      <view class="stats-bar">
        <view
          ...
          :key="index"
        >
          <text class="stats-num" :class="{ animate: showStats }">{{
            stat.value
          }}</text>
          <text class="stats-label">{{ stat.label }}</text>
          <text class="stats-trend" v-if="stat.trend"
      ...
      <view class="achievement-bar">
        ...
        >
          <view
            class="progress-ring"
            :style="{ '--progress': `${achievement.progress * 3.6}deg` }"
          >
        ...
        <view class="card-right">
          ...
          <view class="game-modes">
            ...
            <view
              ...
              :class="getDifficultyClass(game.modes[0].difficulty)"
            >
              难度: {{ "★".repeat(game.modes[0].difficulty)
              }}{{ "☆".repeat(3 - game.modes[0].difficulty) }}
            </view>
          </view>
          <text class="game-bonus"
            >首次通关奖励：{{ game.rewards.firstWin }}积分</text
          >
        </view>
        <view class="card-actions">
          <button
            class="action-btn favorite"
            @tap.stop="toggleFavorite(game.key)"
          >
            <uni-icons
              :type="isFavorite(game.key) ? 'star-filled' : 'star'"
              size="24"
              :color="isFavorite(game.key) ? '#FFD700' : '#999'"
            ></uni-icons>
          </button>
    ...
    <view v-if="showDetails" class="details-popup">
      <view class="popup-content">
        ...
        <view class="button-group">
          <button class="start-button" @tap="startGame(selectedGame.path)">
            开始游戏
          </button>
    ...
    <!-- 帮助中心展示 -->
    <button @tap="openHelp" class="help-button">帮助中心</button>
    <!-- 添加帮助中心按钮 -->
    ...
    return {
      ...
      games: {
        flags: {
          title: "旗帜挑战",
          description:
            "探索世界各国的旗帜文化，从简单的国旗识别到深入了解旗帜的历史和含义。",
          modes: [
            {
              name: "初级模式",
              desc: "认识基本的国家旗帜",
        ...
        capitals: {
          title: "首都挑战",
          description:
            "测试你对世界各国首都的认知，从基础的首都匹配到深入的历史文化知识。",
          modes: [
            {
              name: "首都配对",
              desc: "将国家与其首都正确匹配",
        ...
        timedChallenge: {
          title: "限时挑战",
          description:
            "在限定时间内回答尽可能多的地理问题，考验你的知识储备和反应速度。",
          modes: [
            {
              name: "快速答题",
              desc: "60秒内答题越多分数越高",
        ...
        .map(([key, game]) => {
          ...
          if (recentGame) {
            const daysSinceLastPlayed =
              (new Date() - new Date(recentGame.lastPlayed)) / 86400000;
          ...
          const difficultyMatch = Math.abs(
            userLevel - game.modes[0].difficulty
          );
